...
def main():
	...
	print("lists done")
	filename = raw_input("Enter password file: ")
	usernames, pwd2Crack = loginParser(filename)
	#print(usernames)
	#print(pwd2Crack)
	print("beginning cracking")
	crackedPWDs = crackingMachine(pwd2Crack, firstlist, secondlist, thirdlist, fourthlist, fifthlist,"crackedPasswords.txt")
	#print(crackedPWDs)
...
def crackingMachine(passwords2Crack, rule1, rule2, rule3, rule4, rule5, outfile):
	pwdCracked = []
	out = open(outfile, 'w')
	for i in range(len(passwords2Crack)):
		...
		output = str(passwords2Crack[i]) + ":" + str(pwd)
		print(output)
		out.write(output + "\n")
	out.close()
	return pwdCracked
